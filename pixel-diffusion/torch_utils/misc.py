...
class InfiniteSampler(torch.utils.data.Sampler):
    def __init__(self, dataset, rank=0, num_replicas=1, shuffle=True, seed=0, window_size=0.5, s_max=None):
        ...
        # this is updated every time we yield an image. It is useful only when keep_schedule is True.
        # the purpose is to first sample a sigma and then an image based on that sigma.
        self.sampled_sigmas = {}
        self.buffer_factor = (1 + 1/(s_max - 1))**0.5 if s_max is not None else 1

        print(f'InfiniteSampler s_max={s_max} buffer_factor={self.buffer_factor}')
    ...
    def __iter__(self):
        ...
        while True:
            ...
            if idx % self.num_replicas == self.rank:
                filename = self.dataset._image_fnames[order[i]] # Is correct ???
                # print(filename)
                sample_annotation = self.dataset.annotations.get(filename, (0.0, 300.0))
                # print(filename, sample_annotation)
                ...
                if sample_sigma_min >= 0:
                    is_valid_sample = (sigma > sample_sigma_min*self.buffer_factor) or (sigma < sample_sigma_max)
                else:
                    is_valid_sample = (sigma < sample_sigma_max) and (sigma > 0.2)
                if is_valid_sample:
                    self.sampled_sigmas[filename] = sigma
                    # print('yielding')
                    yield order[i]
                    # print(order[i], 'yielded')
                    # get a new sigma now that you have yielded an image
                    rnd_normal = np.random.normal(0, 1)
                    sigma = np.exp(rnd_normal * 1.2 - 1.2)
            if window >= 2:
...
def print_module_summary(module, inputs, max_nesting=3, skip_redundant=True, verbose=True):
    assert isinstance(module, torch.nn.Module)
