...
def training_loop(
    ...
    ddp = torch.nn.parallel.DistributedDataParallel(net, device_ids=[device], broadcast_buffers=True, find_unused_parameters=False)
    ...
    stats_jsonl = None
    net_dtype = next(ddp.parameters()).dtype
    while True:
        ...
        for round_idx in range(num_accumulation_rounds):
            with misc.ddp_sync(ddp, (round_idx == num_accumulation_rounds - 1)):
                ...
                images = dataset_item["image"].to(device=device, dtype=net_dtype)    
                labels = dataset_item["label"].to(device=device, dtype=net_dtype)  # attention: this is NOT the label for good or bad image. This is more like a class label (dog, cat, etc.)
                ...
                sigma_min = torch.tensor([x[0] for x in batch_annotations], device=device)
                sigma_max = torch.tensor([x[1] for x in batch_annotations], device=device)
                ...
                sigma_tn = torch.where(sigma_t < sigma_max, torch.zeros_like(sigma_min), sigma_min)

                x0, augment_labels = augment_pipe(images) if augment_pipe is not None else (images, None)
                x_tn = x0 + dataset_item["noise"].to(device).to(images.dtype) * sigma_tn[:, None, None, None] * float(add_noise == True)
